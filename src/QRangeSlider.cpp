...
QRangeSlider::QRangeSlider(QWidget *parent) : QWidget(parent), m_lowValue(m_minimum), m_highValue(m_maximum)
{
...
void QRangeSlider::setMinimum(const unsigned int minimum)
{
    if (m_minimum != minimum)
    {
        ...
        emit(rangeChange(m_lowValue, m_highValue));
    }
}
...
{
    return QSize(10 * HANDLE_SIZE + 2 * PADDING, 2 * HANDLE_SIZE + 2 * PADDING);
}
...
void QRangeSlider::setOrientation(Qt::Orientation orientation)
{
    if (orientation != Qt::Horizontal) {
        qWarning() << "QRangeSlider supports only horizontal orientation.";
        return;
    }
}
...
void QRangeSlider::mousePressEvent(QMouseEvent *e)
{
    if (e->position().y() >= (height() - SLIDER_HEIGHT - HANDLE_SIZE) / 2 && e->position().y() <= (height() - SLIDER_HEIGHT + HANDLE_SIZE) / 2) // Check if event was on slider
    {
        ...
        // Check if the mouse press is on the lower handle
        if (mouseX >= (PADDING + ((width() - 2 * PADDING) * (m_lowValue - m_minimum) / (m_maximum - m_minimum)) - HANDLE_SIZE / 2) &&
            mouseX <= (PADDING + ((width() - 2 * PADDING) * (m_lowValue - m_minimum) / (m_maximum - m_minimum)) + HANDLE_SIZE / 2))
        {
            m_grabbedHandle = Handle::Low;
        }
        // Check if the mouse press is on the upper handle
        else if (mouseX >= (PADDING + ((width() - 2 * PADDING) * (m_highValue - m_minimum) / (m_maximum - m_minimum)) - HANDLE_SIZE / 2) &&
                 mouseX <= (PADDING + ((width() - 2 * PADDING) * (m_highValue - m_minimum) / (m_maximum - m_minimum)) + HANDLE_SIZE / 2))
        {
            m_grabbedHandle = Handle::High;
        }
        // Check if the mouse press is on the range bar
        else if (mouseX > (PADDING + ((width() - 2 * PADDING) * (m_lowValue - m_minimum) / (m_maximum - m_minimum))) &&
                 mouseX < (PADDING + ((width() - 2 * PADDING) * (m_highValue - m_minimum) / (m_maximum - m_minimum))))
        {
            m_grabbedHandle = Handle::Range;
        }
    }
}
...
void QRangeSlider::mouseMoveEvent(QMouseEvent *e)
{
    if (m_mousePressed)
    {
        ...
        if (m_grabbedHandle == Handle::Low)
        {
            if (mouseValue < m_highValue)
                setLowValue(mouseValue);
        }
        else if (m_grabbedHandle == Handle::High)
        {
            if (mouseValue > m_lowValue)
                setHighValue(mouseValue);
        }
        else if (m_grabbedHandle == Handle::Range)
        {
            if (m_lowValue + deltaValue >= m_minimum && m_highValue + deltaValue <= m_maximum)
            {
        ...
        update();
        emit rangeChange(m_lowValue, m_highValue); // Emit range change signal
    }
}
...
void QRangeSlider::paintEvent(QPaintEvent *event)
{
    ...
    painter.drawRoundedRect(PADDING + ((width() - 2 * PADDING) * (m_lowValue - m_minimum) / (m_maximum - m_minimum)) - HANDLE_SIZE / 2,
    ...
    // Draw higher handle
    painter.drawRoundedRect(PADDING + ((width() - 2 * PADDING) * (m_highValue - m_minimum) / (m_maximum - m_minimum)) - HANDLE_SIZE / 2,
                            (height() - HANDLE_SIZE) / 2,
                            HANDLE_SIZE,
                            HANDLE_SIZE,
                            2,
