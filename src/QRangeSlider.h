...
signals:
    ...
    void rangeChange(unsigned int lowValue, unsigned int highValue);
...
private:
    ...
    /* Enum for handle type */
    enum class Handle { None, Low, High, Range };

    /* Currently grabbed handle */
    Handle m_grabbedHandle = Handle::None;

    /* Flag to track mouse pressed state */
